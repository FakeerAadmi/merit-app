...
const LabResultView = ({ t, result, navigate, rR }) => (
  <div ref={rR} className="fade-up dark" style={{margin:'0 -24px',padding:'32px 24px 60px',minHeight:'80vh',background:'var(--darkBg)'}}>
    <div style={{display:'flex',gap:8,marginBottom:24}}>
      <button className="btn btn-s" onClick={()=>navigate('/lab/config')} style={{fontSize:13,padding:'8px 16px',borderRadius:'var(--r-f)'}}><Ic name="back" size={14}/> {t('adjust')}</button>
      <button className="btn btn-s" onClick={()=>navigate('/lab')} style={{fontSize:13,padding:'8px 16px',borderRadius:'var(--r-f)'}}>{t('labHome')}</button>
    </div>
