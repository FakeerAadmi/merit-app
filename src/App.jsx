...
/* ═══════════════════════════════════════════════
   APP — all state lives here, routing via React Router
═══════════════════════════════════════════════ */
export default function App() {
  ...
  const fCa    = CATS
    .filter(cc => cc.id !== 'adult' || adultMode)
    .filter(cc => (T[lang]?.[cc.tk] || '').toLowerCase().includes(catQ.toLowerCase())
      || cc.sub.some(s => s.toLowerCase().includes(catQ.toLowerCase())));
  ...
  const callAI = async (prompt) => {
    if (!apiKey) {
      setError(lang === 'mr' ? 'कृपया सेटिंग्जमध्ये Gemini API की सेट करा.' : 'Set your Gemini API key in settings.');
      return;
    }
...
{"recs":[{"name":"Exact Brand Model Full Name as sold in India","price":"numeric INR e.g. 45000","badge":"emoji + 3-word label","summary":"2-3 sentence expert overview including India-specific context and priority match","specs":["Spec 1: detailed value","Spec 2: detailed value","Spec 3: detailed value","Spec 4: detailed value","Spec 5: detailed value","Spec 6: detailed value"],"pros":["Specific concrete strength 1","Specific concrete strength 2","Specific concrete strength 3"],"cons":["Honest specific limitation 1","Honest specific limitation 2"],"target":"Ideal for: specific user type in 20 words","buy":"Best platform: Amazon/Flipkart/Brand store/Croma","brand_domain":"brand.com","warranty":"Warranty as sold in India","rating":"e.g. 4.3/5 (2000+ reviews)"}],"advice":["Specific actionable India buying tip 1","Tip 2","Tip 3","Tip 4"],"comparison":"3-4 sentence expert verdict. Mention which is the clear winner for which buyer type.","insider":"Non-obvious insider fact about this category in India that dealers rarely disclose."}`;
    callAI(pr).then(() => { navigate('/result'); setTimeout(() => rR.current?.scrollIntoView({ behavior: 'smooth' }), 100); });
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--sur)' }}>
      <Navbar
        t={t} lang={lang} setL={setL}
        city={city} saved={saved}
        pathname={pathname}
        onLogoClick={() => { navigate('/'); setCatQ(''); setError(''); }}
        onNavClick={(path) => { navigate(path); setError(''); }}
        ...
        <Routes>
          <Route path="/" element={
            <HomeView t={t} navigate={nav}
              setCat={setCat} setSub={setSub} setPris={setPris}
              setBudget={setBudget} setNotes={setNotes}
              setResult={setResult} setError={setError} setShowT={setShowT}
              setQS={setQS} setQA={setQA}
              apiSaved={apiSaved} TRENDING={TRENDING}
            />
          }/>

          <Route path="/browse" element={
            <BrowseView t={t} lang={lang}
              catQ={catQ} setCatQ={setCatQ} fCa={fCa}
              setCat={setCat} navigate={nav}
              setSub={setSub} setPris={setPris} setBudget={setBudget}
              setNotes={setNotes} setResult={setResult} setError={setError} setShowT={setShowT}
            />
          }/>

          <Route path="/configure" element={co ? (
            <ConfigureView t={t} lang={lang}
              cat={cat} co={co} conf={conf}
              sub={sub} setSub={setSub}
              pris={pris} togP={togP}
              budget={budget} setBudget={setBudget} bds={bds}
              hh={hh} setHh={setHh} showHH={showHH}
              notes={notes} setNotes={setNotes}
              error={error} loading={loading}
              getRec={getRec} navigate={nav}
              setCatQ={setCatQ} setShowT={setShowT} city={city}
            />
          ) : <RedirectHome nav={nav}/>}/>

          <Route path="/result" element={
            <ResultView t={t} lang={lang}
              cat={cat} co={co} sub={sub} budget={budget} hh={hh} city={city}
              pR={pR} result={result}
              activeTile={activeTile} setActiveTile={setActiveTile}
              togS={togS} saved={saved}
              navigate={nav} setCatQ={setCatQ}
              showT={showT} setShowT={setShowT} rR={rR} error={error}
            />
          }/>

          <Route path="/saved" element={
            <SavedView saved={saved} compSel={compSel} togC={togC} togS={togS} navigate={nav}/>
          }/>

          <Route path="/compare" element={
            <CompareView compSel={compSel} navigate={nav}/>
          }/>

          <Route path="/settings" element={
            <SettingsView t={t} lang={lang} setL={setL}
              apiKey={apiKey} setApiKey={setApiKey} apiSaved={apiSaved} saveK={saveK}
              navigate={nav}
              adultMode={adultMode} setAdultMode={setAdultMode} setShowAgeGate={setShowAgeGate}
              cityQ={cityQ} setCityQ={setCityQ} showCD={showCD} setShowCD={setShowCD}
              selC={selC} fCi={fCi}
            />
          }/>

          <Route path="/lab" element={
            <LabView t={t} navigate={nav}
              setLabMode={setLabMode} setPcP={setPcP} setPcB={setPcB}
              setPcU={setPcU} setLabN={setLabN} setResult={setResult} setError={setError}
            />
          }/>

          <Route path="/lab/config" element={
            <LabConfigView t={t} lang={lang}
              labMode={labMode} pcP={pcP} setPcP={setPcP}
              pcB={pcB} setPcB={setPcB} pcU={pcU} setPcU={setPcU}
              labN={labN} setLabN={setLabN}
              loading={loading} error={error} getLabR={getLabR} navigate={nav}
            />
          }/>

          <Route path="/lab/result" element={
            <LabResultView t={t} result={result} navigate={nav} rR={rR}/>
          }/>

          <Route path="/tools" element={
            <ToolsView t={t} navigate={nav}
              setQS={setQS} setQA={setQA}
              setCW={setCW} setCH={setCH}
              setDP={setDP} setDPr={setDPr}
              setResult={setResult} setError={setError} setCompSel={setCompSel}
            />
          }/>

          <Route path="/quiz" element={
            <QuizView t={t} isResult={false} navigate={nav}
              qS={qS} setQS={setQS} qA={qA} setQA={setQA}
              quizQs={quizQs} loading={loading} error={error} runQ={runQ} result={result}
            />
          }/>

          <Route path="/quiz/result" element={
            <QuizView t={t} isResult={true} navigate={nav}
              qS={qS} setQS={setQS} qA={qA} setQA={setQA}
              quizQs={quizQs} loading={loading} error={error} runQ={runQ} result={result}
            />
          }/>
          ...
          <Route path="/calc" element={
            <CalcView t={t} navigate={nav}
              cW={cW} setCW={setCW} cH={cH} setCH={setCH}
              cR={cR} setCR={setCR} mC={mC} wts={wts}
            />
          }/>

          <Route path="/deal" element={
            <DealCheckView t={t} isResult={false} navigate={nav}
              dP={dP} setDP={setDP} dPr={dPr} setDPr={setDPr}
              loading={loading} error={error} chkD={chkD} result={result}
            />
          }/>

          <Route path="/deal/result" element={
            <DealCheckView t={t} isResult={true} navigate={nav}
              dP={dP} setDP={setDP} dPr={dPr} setDPr={setDPr}
              loading={loading} error={error} chkD={chkD} result={result}
            />
          }/>
          ...
          {/* Catch-all */}
          <Route path="*" element={<RedirectHome nav={nav}/>}/>
        </Routes>
        ...
        {showAgeGate && (
          <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,.6)',backdropFilter:'blur(20px)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',padding:20,animation:'fadeUp .2s ease-out'}}>
            <div className="card nm-flat" style={{maxWidth:440,width:'100%',padding:32,background:'var(--sur)',borderRadius:'var(--r-xl)',boxShadow:'0 32px 80px rgba(0,0,0,.3)',border:'1px solid var(--out)'}}>
              ...
              <p style={{fontSize:14,color:'var(--on-sur-v)',lineHeight:1.6,marginBottom:28}}>This section contains products intended for adults (18+). By proceeding, you confirm that you are of legal age to view this content.</p>
...
function RedirectHome({ nav }) {
  React.useEffect(() => { nav('/'); }, [nav]);
  return null;
}
